...
df = df.rename(columns={"Topic Name (Post Forced)": "Cluster", "title": "AbstractTitle"})
df.columns = df.columns.str.strip()
if "Topic (Post Forced)" not in df.columns:
    df["Topic (Post Forced)"] = 0
df["Topic (Post Forced)"] = df["Topic (Post Forced)"].fillna(0).astype(int)

df = pd.merge(df, data, on='AbstractTitle', how='left')

bt60_names = pd.read_csv(names_url)
bt60_names = bt60_names.rename(columns={"title": "AbstractTitle"})
bt60_names.columns = bt60_names.columns.str.strip()
if "Topic (Post Forced)" not in bt60_names.columns:
...
labels = alt.Chart(bar_data).mark_text(align='left', dx=3, color='black').encode(
    y=alt.Y('TopicName:N', sort=alt.SortField('TotalChange', order='descending')),
    x='TotalChange:Q',
    text='Label:N'
)

final_chart = (bar_chart + labels).properties(width=900, height=600, title="Total Change in Abstracts (Top 10)")
